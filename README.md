...
`additional_config["turboquant"]` で渡せる全 key (全て省略可能):
...
備考:
- **`BP=0` は `kind="rht"` 限定で安全**。 block-diag 系 (planar/rotor/iso_fast/iso_full) は boundary 層を gaussian 化しきれないので `boundary_protect="fp8"` か `1` を併用してください (詳細は次節)。
...
`K, V ∈ {1, 2, 3, 4}` の **全 16 通り**を `kv_cache_dtype="turboquant_k{K}v{V}_nc"` の形式で **そのままプリセット名として指定**できます (環境変数を併用する必要なし)。
...
4. `v1_backend.py` の bit override path が environ を読んで、 量子化 kernel に実効 bit 幅を渡す。 ホストプリセットの slot 内に **(K bits / V bits) の packed indices + 残り未使用バイト** という形で書き込む。

#### 後方互換 (環境変数 override の旧 API)

旧来の env var 直接指定もそのまま動きます。 `kv_cache_dtype` をストックプリセット (`turboquant_4bit_nc` 等)にして、 `TURBOQUANT_KEY_BITS` / `TURBOQUANT_VALUE_BITS` で上書き:

```bash
TURBOQUANT_KEY_BITS=2 vllm serve ... --kv-cache-dtype turboquant_4bit_nc   # K=2, V=preset(=4)
```

env var が設定されている場合はそちらが優先 (`setdefault` を使っているため、 ユーザーが明示的に渡した env を新 preset の挙動が上書きすることはない)。
...
- スロットの ceiling サイズはホストプリセット由来なので、 たとえば `turboquant_k2v2_nc` を選んでも slot 自体は `turboquant_3bit_nc` (3-bit slot)を使用し、 実データ部分は 2-bit + 余白。 物理メモリの完全な節約より、 **指定 bit 幅で量子化エラーを抑える品質チューニング**としての側面が強い。
- 1-bit V (`turboquant_k*v1_nc`) は uniform per-vector min/max の 2 levels となり、 GSM-8K のような構造化タスクでは accuracy が急落 (Gemma 4 31B + K=2, V=1 で 48%、 K=V=1 で 0%)。 実用下限は **K2V2** (95% accuracy, 14.6× compression)。
